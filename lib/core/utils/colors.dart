import 'package:flutter/material.dart';

abstract class ColorsStyles {
  static const Color black = Color(0xff000000);
  static const Color white = Color(0xffFFFFFF);
  static const Color homeBackGround = Color(0xff1A4A4C);
  static const Color goldenColor = Color(0xffF6AF58);
  static const Color buttonColor = Color(0xff1C5153);
  static const Color optionsAndParyersTimesColor = Color(0xffEEEEEE);
  static const Color prayerTimesCard = Color(0xff5D8082);
  static const Color tasbihViewBackGround = Color(0xff102A2B);
  static const Color doaCard = Color.fromARGB(255, 229, 232, 240);
  static const Color doaAppBar = Color(0xff061D1C);
}
